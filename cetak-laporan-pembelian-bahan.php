<?php 
    ...
    $sql    = mysqli_query($conn, "SELECT * FROM tbelibahan WHERE month(tgl_beli) = '$bulan' AND year(tgl_beli) = '$tahun' ");
?>
...
<head>
    ...
    <title>Cetak Laporan Pembelian Bahan</title>
                                ...
                                <table class="table table-bordered text-right">
                                    <thead>
                                        <tr class="text-capitalize">
                                            ...
                                            <th class="text-left" style="width: 20%;">Kode Bahan</th>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td class="text-left"><?= $data["no_bahan"] ?></td>
                                                    <?php $join = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM mbahan WHERE kd_bahan = '$data[no_bahan]'")) ?>
                                                    <td class="text-left"><?= $join["nm_bahan"] ?></td>
                                                    <td class="text-left"><?= date("d-m-Y", strtotime($data["tgl_beli"])) ?></td>
                                                    <td><?= number_format($data["hbeli"],0) ?></td>
                                                    <td><?= $data["jumbeli"] ?></td>
