...
<?php
                        ...
                        IF (isset($_GET['id_pelanggan'])){
                            $select_kode= $_GET['id_pelanggan'];
                        }else{
                            echo "<div class='alert alert-info text-center'>Tidak Ada Data Yang Dipilih</div>";
                        }
                        
                        $query  ="SELECT * FROM mpelanggan where id_pelanggan = '$select_kode'";
               
                        $sql    = mysql_query($query);
                        $record  = mysql_fetch_array($sql);
                        
                       	$id 			=$record['id_pelanggan'];
                       	$nama 			=$record['nama_p'];
           				$alamat 		=$record['alamat_p'];
           				$telepon 		=$record['telp_p'];
                        ...
                        $query = "UPDATE mpelanggan SET nama_p='$nm', alamat_p='$al', telp_p='$tp' WHERE id_pelanggan='$select_kode'";
                        $sql   = mysql_query($query);
                            IF ($sql) {
 ...
 <div class="widget-header">
  ...
  <div class="widget-content">
   ...
   <form class="form-horizontal row-border" action="" method="post">
    ...
    <div class="col-md-10"><input type="text" name="idp" class="form-control" readonly="" placeholder="ID Pelanggan" value="<?php echo $id?>"></div> 
    </div> 
    ...
    <input class="form-control" type="text" name="nama1" placeholder="Masukkan Nama Tukang" required="" value="<?php echo $nama ?>" autocomplete="off"></div>
      ...
      <div class="form-group">
       ...
       <div class="col-md-10"><input class="form-control" type="text" name="alamat1" value="<?php echo $alamat?>" placeholder="Masukkan Alamat Tukang" autocomplete="off" required="">
       </div> 
       </div> 
       <div class="form-group">
